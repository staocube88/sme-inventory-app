...
<title>Order History- IVSM</title>
</head>
...
<div class="body-div">
    ...
    <div class="page-name-div">
        <div class="page-name">
            <h1><i class="fa fa-history"> </i> Order History between <?php echo $date_from;?> and <?php echo $date_to;?></h1>  
        </div>
    </div>
        
    <div class="date-history-div">
    <form method="post" enctype="multipart/form-data" action="order-history-search-result.php">  
        <div class="date-div">
        FROM:
        <input id="" name="date_from" value="<?php echo $date_from;?>" type="date" class="text-field"  placeholder="" title="PICK A DATE" value="" required>
        TO:
        <input id="" name="date_to" value="<?php echo $date_to;?>" type="date" class="text-field" placeholder="" title="PICK A DATE" value="" required>
...
<table  class="list-table">
    ...
    <?php
    $orderquery = mysqli_query ($conn,"SELECT * FROM `order_tab`  WHERE staff_id = '$loguser_id' AND DATE(date_entered) BETWEEN '$date_from' AND '$date_to'") or die ('cannot select order');
    $count=0;

...
<div class="body-div">
    ...
    <form method="post" enctype="multipart/form-data" action="order-history-search-result.php">  
      <div class="date-div">
        FROM:
        <input id="" name="date_from" value="<?php echo $date_from;?>" type="date" class="text-field"  placeholder="" title="PICK A DATE" required>
        TO:
        <input id="" name="date_to" value="<?php echo $date_to;?>" type="date" class="text-field" placeholder="" title="PICK A DATE"  required>
...
<table  class="list-table">
    ...
    <?php
    $orderquery = mysqli_query ($conn,"SELECT * FROM `order_tab` WHERE DATE(date_entered) BETWEEN '$date_from' AND '$date_to'") or die ('cannot select order');
    $count=0;

...
<div class="body-div">
    ...
    <div class="page-name-div">
        <div class="page-name">
            <h1><i class="fa fa-history"> </i> Stock Sold Report History</h1>  
        </div>
    </div>
        
    <div class="date-history-div">
    <form method="post" enctype="multipart/form-data" action="stock-sold-report-history-search-result.php?vd_id=<?php echo $vd_id?>">         
        <div class="date-div">
        FROM:
        <input id="" name="date_from" type="date" class="text-field"  placeholder="" title="PICK A DATE" value="<?php echo $date_to;?>" required>
        TO:
...
<table  class="list-table">
    ...
    <?php
    $loadproductquery = mysqli_query ($conn,"SELECT * FROM `voucher_cart_tab` WHERE vd_id='$vd_id' AND order_status_id='S' AND DATE(date_entered) BETWEEN '$date_from' AND '$date_to'") or die ('cannot select voucher cart tab');
    $count=0;
	while($loadproductdata=mysqli_fetch_array($loadproductquery)){
        $count++;
        $vd_id=$loadproductdata['vd_id'];
        $order_id=$loadproductdata['order_id'];
        $staff_id=$loadproductdata['staff_id'];
        $quantity=$loadproductdata['order_qty'];
